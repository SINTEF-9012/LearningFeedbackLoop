...
def compute_rfft_multichannel(
    segs_by_channel: Optional[Dict[str, np.ndarray]] = None,
    fs: float = 1.0,
    ...
    # --- legacy API (tests + older callers) ---
    data_dict: Optional[Dict[str, Any]] = None,
    channels: Optional[List[str]] = None,
    nfft: Optional[int] = None,
) -> Any:
    """Compute a single-sided RFFT for multiple channels.

    This function supports two call styles:

    1) **Current API** (used by `backend/fft_streamer.py`)
       - Provide `segs_by_channel={channel: np.ndarray_segment}`.
       - Returns `(freqs: np.ndarray, spectra: Dict[str, np.ndarray])`.

    2) **Legacy API** (used by tests and older code)
       - Provide `data_dict={channel: List[float]}` plus `channels=[...]` and `nfft=...`.
       - Returns `{channel: {"freqs": [...], "mags": [...]}}`.

    The math/scaling for the current API remains unchanged.
    """

    # --- Legacy API path -------------------------------------------------
    if data_dict is not None:
        selected = _pick_channels(data_dict, channels)
        if not selected:
            return {}

        nfft_local = int(nfft) if nfft is not None else None
        out: Dict[str, Dict[str, List[float]]] = {}
        for ch in selected:
            x = np.asarray(data_dict[ch], dtype=float)
            if nfft_local is not None:
                if x.size < nfft_local:
                    # Zero-pad to requested FFT size
                    x_pad = np.zeros(nfft_local, dtype=float)
                    x_pad[: x.size] = x
                    x = x_pad
                else:
                    x = x[:nfft_local]
    ...
    # --- Current API path ------------------------------------------------
    if segs_by_channel is None:
        raise TypeError("Must provide either segs_by_channel (current API) or data_dict (legacy API)")
