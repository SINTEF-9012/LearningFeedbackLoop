...
from backend.agents.core.schemas import Memory, PatternKey, NumericMetrics
...
class TestMemoryGet:
    ...
    def test_get_backfills_pattern_kind_and_preserves_confidence(self, memory_store):
        mem = Memory(
            id="kind-mem-001",
            session_id="session-kind",
            time_range=(0.0, 5.0),
            pattern_keys=[
                PatternKey(
                    key="signature:spindle_shift_phase_change",
                    confidence=0.42,
                    additional={"source_metric": "phase_differences"},
                ),
            ],
        )

        memory_store.create(mem)
        retrieved = memory_store.get(mem.id)

        assert retrieved is not None
        assert retrieved.pattern_keys[0].confidence == pytest.approx(0.42, rel=1e-6)
        assert retrieved.pattern_keys[0].additional is not None
        assert retrieved.pattern_keys[0].additional["kind"] == "signature"
        assert retrieved.pattern_keys[0].additional["source_metric"] == "phase_differences"

    def test_get_similar_with_resolution_returns_shared_patterns_and_feedback(self, memory_store):
        past = Memory(
            id="mem-past",
            session_id="session-history",
            time_range=(0.0, 5.0),
            annotation_text="Operator stopped and inspected the edge",
            label="tool_break",
            pattern_keys=[
                PatternKey(
                    key="hypothesis:workpiece_slip",
                    confidence=0.81,
        ...
        memory_store.create(past)
        memory_store.create(current)
        memory_store.add_feedback_event(
            memory_id="mem-past",
            action="confirm",
            user_id="operator",
            pattern_keys=["signature:spindle_shift_phase_change"],
            data={"comment": "changed tool after inspection"},
        )

        out = memory_store.get_similar_with_resolution("mem-current", k=5)

        assert len(out) == 1
        entry = out[0]
        assert entry["id"] == "mem-past"
        assert entry["label"] == "tool_break"
        assert entry["shared_pattern_keys"] == ["signature:spindle_shift_phase_change"]
        assert entry["shared_pattern_details"] == [
            {
                "key": "signature:spindle_shift_phase_change",
                "query_strength": pytest.approx(0.43, rel=1e-6),
                "query_source_metric": "phase_differences",
                "candidate_strength": pytest.approx(0.81, rel=1e-6),
                "candidate_source_metric": "phase_differences",
            }
        ]
        assert entry["feedback"]["confirm_count"] == 1
        assert entry["feedback"]["last_action"] == "confirm"
        assert entry["feedback"]["last_comment"] == "changed tool after inspection"

    def test_get_similar_with_resolution_prefers_edge_strength_over_serialized_confidence(self, memory_store):
        past = Memory(
            id="mem-past-edges",
            session_id="session-history",
            time_range=(0.0, 5.0),
            annotation_text="Past signature event",
            pattern_keys=[
                PatternKey(
                    key="hypothesis:workpiece_slip",
                    confidence=0.81,
        ...
        with memory_store._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE memories SET pattern_keys_json = ? WHERE id = ?",
                (
                    json.dumps([
                        {
                            "key": "hypothesis:workpiece_slip",
                            "pattern_type": "custom",
                            "confidence": 0.12,
                            "source_metric": "overridden_metric",
                        }
                    ]),
                    "mem-past-edges",
                ),
        ...
        out = memory_store.get_similar_with_resolution("mem-current-edges", k=5)

        assert len(out) == 1
        detail = out[0]["shared_pattern_details"][0]
        assert detail["candidate_strength"] == pytest.approx(0.81, rel=1e-6)
        assert detail["candidate_source_metric"] == "phase_differences"
